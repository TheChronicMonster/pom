...
const startMarket = async () => {
  ...
  const runMarket = async (who) => {
    console.log(
      `the first price notification is ${stdlib.formatCurrency(price, 4)}`
    );
    ...
    console.log(
      `${who} ${buySell < 1 ? "buys" : "sells"} for ${stdlib.formatCurrency(
        price
      )}.`
    );
    ...
    if (buySell < 1) {
      ...
      try {
        const [lastStaker, price] = await ctc.apis.Staker.buy(buy);
        console.log(`${lastStaker} was the last staker.`);
        console.log(`${price} was the previous price.`);
      ...
      try {
        const [lastStaker, price] = await ctc.apis.Staker.sell(sell);
        console.log(`${lastStaker} was the last staker.`);
        console.log(`${price} was the previous price.`);
    ...
    console.log(
      `${who} ${buySell < 1 ? "bought" : "sold"} at ${stdlib.formatCurrency(
        price
      )}`
    );
